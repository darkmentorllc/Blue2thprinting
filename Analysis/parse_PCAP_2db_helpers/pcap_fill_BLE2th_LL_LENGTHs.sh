...
# This script will process /tmp/LL_LENGTHs_uniq.csv and output /tmp/LL_LENGTHs_uniq_done.csv, which will have cut down the device_bdaddr field to only the master or slave field based on whichever was actually responsible for sending the LL_LENGTH_REQ/RSP
...
python3 post-process_pcap_LL_LENGTHs.py
