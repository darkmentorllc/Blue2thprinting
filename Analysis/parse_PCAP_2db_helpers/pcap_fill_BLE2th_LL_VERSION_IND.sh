...
if [ $? != 0 ]; then
    ...
    tshark -r "$1" -Y '!(_ws.malformed) && !(btle.crc.incorrect) && btle.control_opcode == 0x0c' -E separator=, -E quote=d -T fields -e btle.control_opcode -e btle.central_bd_addr -e btle.peripheral_bd_addr -e btle_rf.pdu_type -e btle.control.version_number -e btle.control.company_id -e btle.control.subversion_number >> /tmp/LL_VERSION_IND.csv
fi
