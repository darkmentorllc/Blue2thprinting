...
def BTIDES_insert_LMP_VERSION_RES(bdaddr, version, company_id, subversion):
    ...
    if (entry == None):
        ...
        base["LMPArray"] = [ ff_LMP_VERSION_RES(version, company_id, subversion) ]
        ...
        if("LMPArray" not in entry.keys()):
            ...
            entry["LMPArray"].append(ff_LMP_VERSION_RES(version, company_id, subversion))
            ###print(json.dumps(TME.TME_glob.BTIDES_JSON, indent=2))
            return