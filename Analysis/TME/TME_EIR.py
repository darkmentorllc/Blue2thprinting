...
def print_classic_EIR_CID_info(bdaddr):
    ...
    if (len(eir_result) == 0):
        ...
        qprint("\tBTC Extended Inquiry Result Device info:")

    for vendor_id_source, vendor_id, product_id, product_version in eir_result:
        if(vendor_id_source == 1):
            qprint("\t\tVendor ID (BT): 0x%04x (%s)" % (vendor_id, BT_CID_to_company_name(vendor_id)))
        elif(vendor_id_source == 2):
            qprint("\t\tVendor ID (USB): 0x%04x (%s)" % (vendor_id, USB_CID_to_company_name(vendor_id)))
        else:
            qprint(f"\t\t: Error: Unknown vendor_id_source = {vendor_id_source}")
        qprint(f"\t\tProduct ID: 0x%04x" % product_id)
        qprint(f"\t\tProduct Version: 0x%04x" % product_version)
...
def print_PSRM(bdaddr):
    ...
    for (page_scan_rep_mode,) in eir_result:
        ...
        qprint(f"\t\tIn BT Classic Data (DB:EIR_bdaddr_to_PSRM)")
