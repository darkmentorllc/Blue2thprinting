...
echo "mysql import"
#mysql -u user -pa --database='bt' --execute="LOAD DATA INFILE '/tmp/EIR_bdaddr_to_PSRM_CoD_uniq.csv' REPLACE INTO TABLE EIR_bdaddr_to_PSRM_CoD FIELDS TERMINATED BY ',' ENCLOSED BY '\"' LINES TERMINATED BY '\n' (bdaddr, @page_scan_rep_mode, @class_of_device) SET class_of_device = CAST(CONV(REPLACE(@class_of_device, '0x', ''), 16, 10) AS UNSIGNED), page_scan_rep_mode = CAST(CONV(REPLACE(@page_scan_rep_mode, '0x', ''), 16, 10) AS UNSIGNED);"
