...
USERNAME="$SUDO_USER"
# macOS's stock readlink lacks -f; use pwd -P to resolve symlinks instead.
...
sudo -u "$USERNAME" bash -lc "cd '$BASE_PATH/Analysis/BTIDES_Schema/rust' && $CARGO_FAST_LTO cargo build --release"
if [ $? -ne 0 ]; then
    echo "cargo build failed in Analysis/BTIDES_Schema/rust. Check the output above."
    exit -1
fi
sudo -u "$USERNAME" bash -lc "cd '$BASE_PATH/Analysis/rust' && $CARGO_FAST_LTO cargo build --release"
if [ $? -ne 0 ]; then
