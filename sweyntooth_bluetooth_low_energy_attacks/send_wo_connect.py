...
while run_script:
    ...
    if data:
        ...
        if (BTLE_SCAN_RSP in pkt or BTLE_ADV in pkt) and pkt.AdvA == advertiser_address.lower() and connecting == False:
            ...
            raw_pkt = bytearray(raw(pkt))
            hex_string = ' '.join(format(byte, '02x') for byte in raw_pkt[6:-3]) # -3 is to remove the 3 bytes of CRC that's at the end. 6 is to get up to the opcode byte
            print(hex_string)
            ...
            print(Fore.GREEN + 'XENO: Got LL_LENGTH_RSP or LL_UNKNOWN_RSP')
            raw_pkt = bytearray(raw(pkt))
            hex_string = ' '.join(format(byte, '02x') for byte in raw_pkt[6:-3])
            print(hex_string)
