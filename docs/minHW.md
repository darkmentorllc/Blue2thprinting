 ...
 * *Note on per-port power switching (PPPS), if you add the Realtek DarkFirmware dongle for LMP fingerprinting:* the DarkFirmware firmware wedges after a few LMP injections, and the only thing that recovers it is a real USB power-cycle. `central_app_launcher.py` performs that cycle with `uhubctl`, which requires a hub that supports **per-port** power switching. Most inexpensive hubs (both the unpowered and the powered one linked above) are "ganged" (all ports share one power line and switch together), so `uhubctl` cannot cycle the Realtek without also bouncing the Sniffle/Better_Getter dongles; in that case the code detects the ganged hub and disables LMP-cycle recovery. The Pi Zero's single data port is taken up by the hub itself, so you also cannot isolate the Realtek on the Pi's PPPS-capable root port while keeping the other dongles attached. To keep LMP capture running continuously, either use a hub from [uhubctl's supported-hardware list](https://github.com/mvp/uhubctl#compatible-usb-hubs) (per-port power switching, with the Realtek on its own port), or run the Realtek dongle on a Raspberry Pi 4, whose built-in VL805 hub is PPPS and not ganged. Note the same limitation also affects the Sniffle and Better_Getter serial dongles: they recover a wedged dongle primarily via `usbreset` (which works even on a ganged hub), and only fall back to a `uhubctl` power-cycle (which a ganged hub disables) as a last resort. So without a PPPS hub, BLE advertisement (Sniffle), GATT (Better_Getter), and HCI (btmon) capture all still work; you lose only the `uhubctl` last-resort recovery for the serial dongles and the DarkFirmware LMP power-cycle recovery. LMP fingerprinting is also an *active* BR/EDR operation (it pages and connects to the target), unlike the otherwise-passive advertisement capture, so weigh that against your engagement's posture before relying on it.
 ...
 * *Note (power):* **more than 2 Sonoff dongles requires a powered, per-port-power-switching (PPPS) hub** (see the hub note above). Each Sonoff draws enough that adding a 3rd on top of the other dongles exceeds what the Pi Zero can supply through the unpowered ganged hub; the symptom is **USB enumeration failures** (`dmesg`: `device not accepting address, error -32` / `unable to enumerate USB device`) with a dongle dropping off the bus, which stalls the Sniffle receiver and can wedge capture entirely (the launcher loops on `usbreset` for the dropped port and never resumes discovery). A powered PPPS hub supplies the current and lets `uhubctl` recover a wedged port cleanly.
