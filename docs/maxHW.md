...
* * *Note on per-port power switching (PPPS) for the Realtek LMP dongle:* the DarkFirmware firmware wedges after a few LMP injections, and the only thing that recovers it is a real USB power-cycle. `central_app_launcher.py` performs that cycle with `uhubctl`, which requires a hub (or port) that supports **per-port** power switching. The inexpensive hub linked above — like most cheap hubs — is "ganged" (all ports share one power line and switch together), so `uhubctl` cannot cycle the Realtek without also bouncing the Sniffle/Better_Getter dongles; the code then detects the ganged hub and disables LMP-cycle recovery. Give the Realtek its own PPPS-capable port so cycling it doesn't power off the other dongles: a hub from [uhubctl's supported-hardware list](https://github.com/mvp/uhubctl#compatible-usb-hubs), a Raspberry Pi 4's built-in VL805 hub (PPPS and not ganged), or — on the x86 build — a motherboard/root port that `uhubctl -l` reports as PPPS-capable. The same fallback also backs up the Sniffle and Better_Getter serial dongles, which otherwise recover a wedged dongle via `usbreset` (that works even on a ganged hub). So without a PPPS port, advertisement (Sniffle), GATT (Better_Getter), and HCI (btmon) capture all still work — you lose only the `uhubctl` last-resort dongle recovery and continuous LMP capture. Also note LMP fingerprinting is an *active* BR/EDR operation (it pages and connects to the target), unlike the otherwise-passive advertisement capture, so weigh it against your engagement's posture.
